...
class MY_Pagination extends CI_Pagination {
    ...
    function create_links()
    {
        ...
        if ($CI->config->item('enable_query_strings') === TRUE OR $this->page_query_string === TRUE)
        {
            $this->base_url .= (strpos($this->base_url, '?') === FALSE ? '?' : '&amp;').$this->query_string_segment.'=';
        }
        else
        {
        ...
        if ( ! empty($get))
        {
            $this->suffix = (($CI->config->item('enable_query_strings') === TRUE OR $this->page_query_string === TRUE) ? '&amp;' : '?').http_build_query($get);
        }
        ...
        {
            $first_url = ($this->first_url == '') ? $this->base_url.(($CI->config->item('enable_query_strings') === TRUE OR $this->page_query_string === TRUE) ? '1' : '') : $this->first_url;
        ...
        if ($this->display_pages !== FALSE)
        {
            ...
            {
                if(($this->two_digits && $this->use_page_numbers) && ($loop < 10 && $loop > 0))
                {
                    $loop = str_pad($loop, 2, '0', STR_PAD_LEFT);
                }
