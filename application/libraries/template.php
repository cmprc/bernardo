...
class CI_Template
{
        ...
        if ($region) { // Display a specific regions contents
            ...
            if ($this->parse_template === true or $parse === true) {
               ...
               // Use CI's loader class to render the template with our output array
                $output = $this->CI->load->view('../'.$this->template_dir.$this->master.EXT, $this->output, $buffer);
   ...
   * @return  void
   */
    public function build($view, $data = array(), $render = false)
    {
      ...
      // Merge in what we already have with the specific data
        ...
        // Check Json LD
        if ($this->json_ld){
            $json_ld = "<script type='application/ld+json'>".$this->json_ld."</script>";
            $this->write('head_scripts', $json_ld, false);
        }
      ...
      // We don't need you any more buddy
        ...
        $this->write_view('content', $view.EXT, $this->data, false, true);

        return $this->render(null, $render);
    }
   ...
   */
    ...
    {
        if ($module) {
            $view = '../../'.$module.'/'.$this->module_views.$view.EXT;
        }
  ...
  */
    ...
    {
        if (isset($this->regions[$name])) {
            return current($this->regions[$name]['content']);
   ...
   */
    ...
    /**
     * Retrieves a metadata that was already seted
     * @author Ralf da Rocha [ralf@ezoom.com.br]
     * @date   2016-10-18
     * @param  [type]     $name Name of the metadata
     * @return [type]           Content form the metadata
     */
    ...
    /**
     * Add json+ld do the template
     * @author Ralf da Rocha [ralf@ezoom.com.br]
     * @date   2016-09-27
     * @param  string|array|object      $json_ld
     * @param  boolean                  $prettify     Set TRUE to not minify the json+ld
     */
