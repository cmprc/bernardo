...
class Image{
    ...
    {
        $this->extensions = array('jpg', 'png', 'jpeg', 'gif', 'bmp', 'JPG', 'PNG', 'JPEG', 'GIF', 'BMP');
    ...
    */
    public function resize($params)
    {
        ...
        } else  if( filemtime($this->cache_dir . "{$file_name}") < filemtime($file_path) ) {
            $q = $ext=='png'?$this->png_comp_level: $q;
    ...
    */
    public function merge($params)
    {
        ...
        $ext = pathinfo($file, PATHINFO_EXTENSION);
        $file_name = md5("{$src}{$wtm}{$w}{$h}{$t}{$l}{$pct}{$q}") . '.' . $ext;
    ...
    */
    public function apply_mask($params)
    {
        ...
        if(!is_file($file_path) || !is_file($file_mask))
        {
            $ext = pathinfo($this->error_image, PATHINFO_EXTENSION);
            $this->wideimage
                ->load($this->error_image)
        ...
        $file_name = md5("{$src}{$mask}{$w}{$h}{$t}{$l}{$pct}{$q}") . '.' . $ext;
